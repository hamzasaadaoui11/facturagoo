...
const StatCard: React.FC<{
    ...
    <div className="bg-white rounded-2xl p-6 shadow-sm ring-1 ring-slate-100 transition-all hover:shadow-md group">
        ...
        <div className="mt-4">
            ...
            <p className="text-2xl font-bold text-slate-900 mt-1">{item.stat}</p>
...
const ShortcutCard = ({ icon: Icon, label, desc, onClick, colorClass }: { icon: React.ElementType, label: string, desc: string, onClick: () => void, colorClass: string }) => {
    return (
        <button onClick={onClick} className="bg-white rounded-2xl p-5 shadow-sm ring-1 ring-slate-100 flex items-center gap-4 text-left hover:ring-emerald-500/50 hover:shadow-md transition-all group">
            <div className={`flex items-center justify-center h-12 w-12 rounded-xl ${colorClass} bg-opacity-10 group-hover:scale-110 transition-transform`}>
...
const Dashboard: React.FC<DashboardProps> = ({ invoices, clients, products, companySettings }) => {
    ...
    // --- New Logic for Top Selling Products ---
        ...
        <div className="space-y-8 animate-fadeIn pb-8">
            ...
            <div className="bg-gradient-to-r from-emerald-900 to-emerald-700 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-bold">Bienvenue, {welcomeName} !</h1>
                        <p className="text-emerald-100 mt-2 text-lg">Voici un aperçu de vos activités aujourd'hui.</p>
                    </div>
                    
                    {/* Widget Date Design (Glassmorphism) */}
                    <div className="bg-white/10 backdrop-blur-md rounded-2xl p-2 pr-5 border border-white/10 shadow-lg flex items-center gap-4">
                        ...
                        <div className="text-white">
                            ...
                            <p className="text-lg font-bold capitalize leading-none">{todayDate}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-6">
                    ...
                    {/* Alertes Stock (Remplace Etat du système) */}
                        ...
                        <div className="divide-y divide-slate-100">
                           {recentClients.map(client => (
                             <div key={client.id} className="p-4 flex justify-between items-center hover:bg-slate-50 transition-colors">
                                ...
                                <div className="text-right">
                                    <p className="text-xs text-slate-400">{client.email || 'Pas d\'email'}</p>
                                </div>
                             </div>
                           ))}
                        </div>
