        ...
        <div className={`fixed inset-0 z-50 flex items-center justify-center transition-opacity duration-200 ${isVisible ? 'opacity-100' : 'opacity-0'}`} aria-modal="true">
            ...
            <div className={`relative w-full max-w-lg p-6 bg-white rounded-lg shadow-xl transition-all duration-200 ease-in-out flex flex-col max-h-[90vh] ${isVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
                ...
                <div className="flex items-start justify-between pb-4 border-b border-neutral-200">
                     <div>
                        <h3 className="text-lg font-semibold text-neutral-900">{isEditMode ? t('editSupplier') : t('newSupplier')}</h3>
                    </div>
                ...
                <div className="mt-6 flex-1 overflow-y-auto custom-scrollbar pr-2">
                    <form id="supplierForm" onSubmit={handleSubmit} className="space-y-5">
                        ...
                        {/* Type Selector */}
                        <div>
                            <label className="block text-sm font-medium text-neutral-700 mb-2">{t('supplierType')}</label>
                        ...
                        {type === 'Entreprise' && (
                            <div className="bg-neutral-50 p-4 rounded-lg border border-neutral-200 space-y-4">
                                <div>
                                    ...
                                    <input type="text" id="company" value={company} onChange={(e) => setCompany(e.target.value)} placeholder={language === 'es' ? "Ej: Empresa Proveedor" : "Ex: Fournisseur SARL"} className="mt-1 block w-full rounded-lg border-neutral-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm" />
                                </div>
                                <div>
                                    <label htmlFor="ice" className="block text-sm font-medium text-neutral-700">{t('ice')}</label>
                                    <input type="text" id="ice" value={ice} onChange={(e) => setIce(e.target.value)} placeholder={language === 'es' ? "Identificador Fiscal" : "Identifiant Commun"} className="mt-1 block w-full rounded-lg border-neutral-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm" />
                                </div>
                            </div>
                        )}
