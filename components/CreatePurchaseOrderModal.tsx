        ...
        <div className={`fixed inset-0 z-50 flex items-center justify-center transition-opacity duration-200 ${isVisible ? 'opacity-100' : 'opacity-0'}`} aria-modal="true">
            ...
            <div className={`relative w-full h-full md:h-auto md:max-h-[95vh] md:max-w-6xl bg-white md:rounded-xl shadow-2xl transition-all duration-200 ease-in-out flex flex-col ${isVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-200">
                    <div>
                        <h3 className="text-lg font-bold text-neutral-900">{orderToEdit ? t('newPurchaseOrder') : t('newPurchaseOrder')}</h3>
                        <p className="text-xs text-neutral-500">{orderToEdit ? `#${orderToEdit.documentId || orderToEdit.id}` : ''}</p>
                    </div>
                ...
                <div className="px-4 md:px-6 py-5 overflow-y-auto custom-scrollbar space-y-6 flex-1">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div className="sm:col-span-1">
                            <label className="block text-sm font-semibold text-neutral-700 mb-1">{t('supplier')} *</label>
                            <select value={supplierId} onChange={(e) => setSupplierId(e.target.value)} className="block w-full rounded-lg border-neutral-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm h-10 bg-white">
                        ...
                        <div><label className="block text-sm font-semibold text-neutral-700 mb-1">{t('dueDate')}</label><input type="date" value={expectedDate} onChange={(e) => setExpectedDate(e.target.value)} className="block w-full rounded-lg border-neutral-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm h-10"/></div>
                    </div>
                    ...
                    <div className="bg-neutral-50 p-4 rounded-xl border border-neutral-200 shadow-inner space-y-4">
                        ...
                        <div className="grid grid-cols-24 gap-3 items-end">
                            ...
                            <div className="col-span-24 lg:col-span-3">
                                ...
                                <select value={tempVat} onChange={(e) => setTempVat(parseInt(e.target.value))} className="block w-full rounded-lg border-neutral-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-xs h-10 bg-white">
                            ...
                            <table className="min-w-full divide-y divide-neutral-200">
                                <thead className="bg-neutral-50">
                                    <tr>
                                        ...
                                        <th className="px-4 py-2 text-left text-[10px] font-bold text-neutral-500 uppercase">{t('total')}</th>
                    ...
                    <div className="flex justify-end border-t border-neutral-200 pt-6">
                        <div className="w-full max-w-xs space-y-3">
                            ...
                            <div className="flex justify-between items-center p-3 bg-emerald-50 rounded-xl ring-1 ring-emerald-100"><span className="text-sm font-bold text-emerald-800">{t('totalTTC')}</span><span className="text-lg font-black text-emerald-700">{totals.totalAmount.toLocaleString('fr-FR', { style: 'currency', currency: 'MAD' })}</span></div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row justify-end gap-3 px-4 md:px-6 py-4 bg-neutral-50 border-t border-neutral-200 md:rounded-b-xl">
                     <button onClick={handleClose} disabled={isSubmitting} className="px-5 py-2.5 text-sm font-bold text-neutral-600 bg-white border border-neutral-300 rounded-lg shadow-sm hover:bg-neutral-50 disabled:opacity-50">{t('cancel')}</button>
                    <button onClick={handleSave} disabled={isSubmitting} className="px-7 py-2.5 text-sm font-bold text-white bg-emerald-600 border border-transparent rounded-lg shadow-md hover:bg-emerald-700 transition-all flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50">{isSubmitting ? <Loader2 size={16} className="animate-spin" /> : <FileText size={16} />} {isSubmitting ? t('save') : t('save')}</button>
                </div>
            </div>
            <style>{`
