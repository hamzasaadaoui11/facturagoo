    ...
    const handleAddItem = () => {
        ...
        if (selectedProductId) {
            ...
            if (product && product.productType === 'Produit') {
                ...
                // Calculate total quantity of this product already in lineItems
                const existingQty = lineItems
                    .filter(item => item.productId === selectedProductId)
                    .reduce((acc, item) => acc + (item.quantity || 0), 0);
                
                if (currentStock < (existingQty + qty)) {
                    alert(`${t('insufficientStock') || 'Stock insuffisant'}. ${t('available') || 'Disponible'}: ${currentStock}`);
                    return;
                }
            }
        }
